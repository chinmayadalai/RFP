...
const instantiateChaincode = async function(peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name, collection_definition_json_filepath) {
	logger.debug('\n\n============ Instantiate chaincode on channel ' + channelName +
		' ============\n');
	logger.debug('\n\n============ config.Org1MSP ' + config.Org1MSP  + ' ============\n');	
	let error_message = null;
	let client = null;
	let channel = null;
	let collectionsConfigPath = null;
	try {
		if(collection_definition_json_filepath != null)
			collectionsConfigPath = path.resolve(__dirname, collection_definition_json_filepath);
		...
		const request = {
			...
			'endorsement-policy': {
			        identities: [
						{ role: { name: 'member', mspId: config.ORG1_MSP }}
